...
for (var _i = 0; _i < ds_list_size(_singletonNameList); _i++)
{
	...
	for (var _a = 0; _a < ds_list_size(_currentVarNameList); _a++)
	{
		...
		if (string_pos("MAP", _currentVariable) > 0)
		{
			var _currentVarSuffix = string_delete(_currentVariable, 1, 3);
			ds_map_replace_map(_currentInstVarMap, _currentVariable, variable_instance_get(_currentSingletonInstance, _currentVarSuffix));
		}
		else
